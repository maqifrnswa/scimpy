...
%8.5x11 PRINTING
%\usepackage[paper=letterpaper, heightrounded]{geometry}

%EDITING:
%\usepackage[paper=letterpaper,layoutsize={6in,9in},bindingoffset=.375in, heightrounded, showframe, showcrop, twoside]{geometry}

%BOOK PRINTING:
\usepackage[papersize={6in,9in},layoutsize={6in,9in},bindingoffset=.375in, heightrounded, twoside]{geometry}
...
© Copyright Scott Howard 2016-2017
...
%Dr. Scott Howard is a professor of electrical engineering at the University of Notre Dame. He received a PhD in Electrical Engineering from Princeton University and was a post-doctoral research associate in Applied and Engineering Physics at Cornell University prior to appointment at Notre Dame.
%
...
Scimpy is a tool to help you design loudspeakers by measuring the loudspeaker impedance, extracting performance information known as ``Thiele-Small Parameters" (TS parameters), and then using these extracted parameters to model and predict speaker performance inside of a box. You can either measure and extract TS parameters yourself, or you can download/import data from vendor websites and use it to design the speaker enclosure.
...
Other good software tools: \url{http://audio.claub.net/software/jbabgy/jbagby.html}

Good diffraction paper: \url{http://www.torean.dk/artikel/Diffraction.pdf} conclusion (from me) Urban's DED model (section 4.10). Diffraction theory can be sommerfeld or fresnel-krichoff. Since you know you hear things at 90 degreeseven with infinite baffle, then sommerfeld doesn't work - use fresnel-kirchoff. You get form of $F_0=\frac{1+\cos(\theta)}{2}$. We know edges will do something to diffraction such that at low frequencies far from the source you get $F_{tot} \,d\phi= F_0\,d\phi + F_n\,d\phi=\text{constant}$, so $F_n=\frac{-\cos(\theta)}{2}$. Doing that, assuming a circular baffle for an approximation, and assuming very far away from speaker and in line with speaker, you get $F_{tot}= 1-\frac{1}{2} e^{-ikb}$ where $b$ is baffle radius ($r_0\approx r_n \gg b$). So $F=1$ with an infinite baffle, want to find $kb$ when $F=1/2$, happens when $\nu=\frac{c}{\pi b}$.that seems wrong: Do we want F=1/2? I think maybe $F^2=1/2; F=1/sqrt(2)$?

Freeware from Tolvan Data (Basta! = speaker simulator, Sirp = measurements, The Edge = edge diffraction modeller, xdir = directional diffraction modeller): \url{http://www.tolvan.com/index.php?page=/basta/basta.php}


\chapter{Usage}

\section{Impedance Testing}

\subsection{Hardware Connections}



To test, hook up hardware as shown in Fig. \ref{hookup}, and described in the following steps:
\begin{figure}
\includegraphics[width=\textwidth]{figures/test-layout.png}
\caption{Scimpy hardware connections} \label{hookup}
\end{figure}
\begin{enumerate}
\item Using alligator clips, connect a speaker driver and test resistor (in the range of 12--100$\Omega$) in series with your speaker driver.
\item Connect both the left and right output (tip \& ring) of the headphones port to the free lead of the test resistor and connect the ground (shield) of the headphones port to the free lead of the speaker
\item Connect the ``right" (ring) of the sound card line in port to the speaker lead that is connected to the test resistor
\item Connect the ``left" (tip) of the sound card line in port to the test resistor lead that is already connected to the headphones port.
\item Connect the ground (shield) of the souncd card line in port to the lead of the speaker already attached to the headphones ground.
\end{enumerate}


Why do we  use this configuration? Headphones output is a low impedance output that expect loads on the order of 10--100$\Omega$, typically \~32$\Omega$. Line out, on the other hand, expects high impedance loads and therefore may not have enough current (line-out impedance is on the order of 100--600$\Omega$). Since we're driving speakers, use headphones out.

As for the input, line in is somewhat low impedance (600--47k$\Omega$), but works for our case. If signals are extra small, you can use the sound card microphone input, which has some extra gain. Microphones are sometimes powered, so you may want or need to use a blocking capacitor if you use the microphone input.

\subsection{How to Use the Software}

\begin{enumerate}
\item Open Scimpy. Click on the ``Impedance Measurements" tab.
\item Choose your sound input and output device. Avoid ``default," it can be prone to some errors due to extra buffering.
\item Use your OS's audio controls to set and control output volume, input gain, and select ``line in" input capture (e.g., ``alsamixer")
\item Review measurement parameters (sample rate, duration). Default values should work fine. Be sure to input what your test resistor value. For accuracy, measure the test resistor with a multimeter.
\item Click ``begin test." Review input data (bottom plot in main window) to make sure there is no clipping (i.e., all data is in between -1 and 1) and adjust sound record/output levels in your OS's control software accordingly accordingly.
\item Click ``Save .ZMA/.ZDA" if you'd like to save your measurements.
\item Open the fitting tab. Enter BL value from the driver spec sheet. If you don't have it, it's ok - this just scales the predicted output power in the next step
\item Click ``Extract TS Parameters." Below are some tips to get good values.
\begin{enumerate}
\item Enter in numbers close to what you expect, then fit for the whole range (20-20000 Hz)
\item OR if you don't know what figures to use to start: first choose minimum and max frequencies around the resonance peak, set K=.00000001, BL=5, and all other variables = 1. Run, re-run several times until the result stops changing. Export it to the speaker modeller to compare against the measured data and see if it makes sense. Then increase the min and max frequencies to run over the full range (20-20000), or at least a bigger range. Some cheap drivers are ugly at low frequencies, so you can choose a min freq that skips those low frequencies.
\end{enumerate} 
\item Click ``Hold plot" on the main plot window.
\item Click ``Export to modeller"
\item Click on the ``Speaker Modelling" tab.
\item At the bottom of the tab, click ``Calculate and plot."
\item Compare your modelled fit to the measured data, they should match.
\item Play with enclosure size and port parameters to see output performance. Clicking on ``infinite baffle" will model the speaker in an infinitely large box. Selecting ``B2 Box" will find the volume of the box for a ``traditional" closed-box geometry (so it behaves like a second-order Butterworth filter). Selecting ``QB3-B4-C4" will find the box volume and port geometry to achieve a ``traditional" vented/ported speaker enclosure alignment. Details are in the theory sections below.
\end{enumerate}
...
As Thiele accurately describes in the above quote, not everything makes a good speaker. You need to get the driver, the enclosure (i.e., cabinet or ``box") tuned just right. This part goes in to the theory to illustrate what goes in to making a good loudspeaker. This is largely a presentation of the ideas from Thiele and Small's papers, but hopefully with some insight.
...
\textbf{Note 4/2/2017: passive crossover networks want an entirely real load. If the load is not real, then an extra frequency-dependent phase is added - which means group delay, which means slower bass and ``beam steering" for two-way systems}
...
\end{align}
which shows we need a another voltage meter to get $\hat{v}_s$. This is shown in Figure~\ref{imp_measure2}, and is the circuit this project uses to find $\hat{Z}_s$:

\begin{align}
\hat{Z_s}=R_{test}\frac{\hat{v}_2}{\hat{v}_1-\hat{v}_2}
\end{align} 
...
\begin{circuitikz}
  ...
  to[short] (1,2)
  to[R=$R_{test}$,i=$\hat{i}_r$,v=$\hat{v}_r$,o-o] (3,2) % The resistor
  to[short] (4,2) node[above] {$\hat{v}_2$}
  to[generic=\mbox{$\hat{Z}_s$},v=$\hat{v}_s$,i=$i_s$] (4,0)
  to[short] (0,0);
  \draw (1,0)
  to[voltmeter,o-o] (1,2)
  node[above] {$\hat{v}_1$};
  \draw (4,2)
  to[short,o-] (5.5,2);
  \draw (4,0)
  ...
  to[short,-o] (4,2);
\end{circuitikz}
\caption{Speaker impedance measurement used in this project, no lock-in required}\label{imp_measure2}
\end{figure}
...
\textbf{NOTE: phase shift between two means there is BEAM STEERING, show radiation patterns from LR paper? \url{http://www.linkwitzlab.com/JAES/jaes_papers76.htm} need delay circuitry if offset from front of cabinet? Overnight sensations: LR2 with extra delay on tweeter and and l-pad attenuator}
...
\begin{circuitikz}
  ...
  to[short] (4,2);
\end{circuitikz}
\caption{Simple Frequency Dependent Electrical Model}\label{electrical_model}
\end{figure}

Since eddy currents are highly non-linearly in frequency (i.e., they are a function of voice coil position), a more complicated model is needed to capture the frequency dependent coil inductance can be used. Leach ("Loudspeaker Voice-Coil Inductance Losses:
Circuit Models, Parameter Estimation, and
Effect on Frequency Response") uses the fact that inductors are a causal system\footnote{Leach uses the Bode Gain-Phase relationship, which describes the relationship between the real and imaginary parts of any causal passive circuit element. This is similar to what the optics community calls the Kamers-Kronig relations.}, which means the voice coil inductor impedance can be modelled as:
\begin{equation}
Z_L=K(i\omega)^n
\end{equation}
as seen in Figure~\ref{electrical_model_leach}.
...
\begin{circuitikz}
  ...
  to[short] (0,0);
\end{circuitikz}
\caption{Frequency Dependent Electrical Model}\label{electrical_model_leach}
\end{figure}


The complex $\hat{Z}_L$ is the same as if you had the circuit in Figure~\ref{electrical_model} with
\begin{align}
R_{eddy}&=\frac{K\omega^n}{\cos(n\pi/2)} & L_e&=\frac{K\omega^{n-1}}{\sin(n\pi/2)}
\end{align}
where $K$ and $n$ are fit parameters. This is great, because we can now characterize the frequency dependent voice-coil impedance with just two parameters, which makes extracting these values from measurements easy.




Since $\hat{Z}_L$ is a function of frequency, you can describe a complete frequency dependent model as Figure~\ref{simple_electrical_model} by using the following terms for the electrical resistance and impedance:
\begin{align}
R_e(\omega)&=R_{e,\text{DC}}+\Re\{\hat{Z}_L\} & L_e (\omega) &= \Im\{\hat{Z}_L\}/\omega
\end{align}
where $R_{e,\text{DC}}$ is the value of $R_e$ at $\omega=0$. This allows us to include frequency dependence in our simple model seen in Figure~\ref{simple_electrical_model}.
...
\begin{circuitikz}
  \draw (0,0)
  to[open,v^<=$v_i$,o-o] (0,2) % The voltage source
  to[R=$R_e(\omega)$] (2,2) % The resistor
  to[L=$L_e(\omega)$] (4,2)
  to[short] (5,2)
  to[generic=$\hat{Z_e}$,i=$i_e$] (5,0)
  to[short] (0,0);
\end{circuitikz}
\caption{Simple Electrical Model}\label{simple_electrical_model}
\end{figure}
...
Variable & Definition & Description \\
\midrule
$Z_e$ & $\frac{(B\ell)^2}{Z_m}$ & Electric ompedance of mech. imp.\\
$\hat{Z}_L$ & $K(i\omega)^n$ & Freq. dependent voice coil imp.\\
$R_e(\omega)$ & $R_e + \Re\{\hat{Z}_L\}$  & Electrical resistance of the voice coil\\
$L_e(\omega)$ & $\Im\{\hat{Z}_L\}/\omega$ & Electrical inductance of the voice coil\\
...
\begin{align}
P_a=\frac{\left| v_o \right|^2S_d^2 \rho}{
