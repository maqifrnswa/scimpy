...
\begin{align}
\lim_{r\rightarrow 0} \left[ 2\pi r^2\left(\frac{k\,K}{\omega \rho r} e^{-ikr} - \frac{iK}{\omega \rho r^2} e^{-ikr}\right) \right] &= A\hat{v}=\hat{Q}_0\\
...
\hat{Z}&=\frac{\hat{p}}{\hat{Q}}=\frac{\omega \rho}{2\pi r(ka-i)}=\frac{\omega^2 \rho }{2\pi c(k^2a^2+1)}+\frac{i \rho \omega }{2\pi r (k^2a^2+1)}
\end{align}
\begin{align}
...
\end{align}
Now we consider $(ka)^2\ll 1$ by assuming $\lambda\gg 2 \pi a$, and by considering wave propagation to the far field $r\gg\frac{\lambda}{2\pi}$
\begin{align}
...
$C_{es}$ & $\frac{(B\ell)^2}{M_{ms}}$ & Equivalent electrical capacitance from diaphragm and voice coil mass\\
$B$ & & Magnetic flux density of speaker driver magnets\\
$\ell$ & & Total length of wire in voice coil\\
$R_{es}$ & $\frac{(B\ell)^2}{R_{ms}+S_d^2Z_{rad}}$ & Equiv. electrical resistance from mechanical dampening in the speaker suspension and front front acoustic radiation\\
$S_d$ & & Speaker driver area\\
$Z_{rad}$ & $\frac{\omega^2 \rho}{2\pi c}$ & 1/2 of acoustic impedance of monopole radiation\\
$L_{ces}$ & $\frac{(B\ell)^2}{C_{ms}}$ & Equivalent electrical inductance of the suspension's mechanical compliance\\
$L_{ceb}$ & $\frac{(B\ell)^2}{S_d^2} \frac{\rho c^2}{V}$ & Equivalent electrical inductance of the compression of air in a box with volume $V$\\
...
$C_{lev}$ & $\frac{(B\ell)^2}{S_d^2}\frac{A}{\rho l}$ & Equivalent electrical capacitance from a mass of air flowing through a vent/port of area $A$ and length $l$\\
$R_{ev}$ & $\frac{(B\ell)^2}{S_d^2\left( \frac{\mu l}{\kappa A}+Z_{rad}\right)}$ & Equiv. electrical resistance of vent/port acoustic radiation and of viscous air flow through a vent/port of area $A$ and length $l$\\
\bottomrule
\end{tabularx}
\caption{Variable Definitions}\label{table:variables}
\end{table}


\textbf{circuit is general, can remove vent, or entire box as would be done in measurements typically}
...
\textbf{output power rev should be maximized, proportional to voltage across it, so rel should be made as small as possible. Same with Res}

\textbf{in vented box, Rel is low (well sealed - acoustic resistance is high), rev is high (air moves easily, not viscous flow through vent), Reb polyfill can be tuned to help increase size of box (more on that next) but can assume Reb large (acoustic resistance is low)}

\textbf{sealed box, rel still tiny}
...
\textbf{thiele does not have factor of two in resistance of vent}

\textbf{$\mu$ for air is $1.56\times10^{-5}$m$^2$/s.}

\textbf{Compliance is V/rho csquared}
